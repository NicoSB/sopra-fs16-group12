...
public class PunchAction implements Action {
    ...
    public PunchAction(UserRepository userRepo, ItemRepository itemRepo, WagonLevelRepository wagonLevelRepo) {
    ...
    public void execute(ActionResponseDTO response) {
        ...
        if (item != null) {
            ...
            if (user.getCharacter() instanceof Cheyenne && item.getItemType() == ItemType.BAG) {
                item.setUser(user);
                item = itemRepo.save(item);
